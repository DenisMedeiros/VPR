...
class BoxVersion(models.Model):
    ...
    name = models.CharField(max_length=20, db_index=True,
        verbose_name='Name', help_text='Box version name.', 
        validators=[
            RegexValidator(
                regex=(
                    '^[a-z0-9](([_](?![_])|([.](?![.])))|[a-z0-9])+[a-z0-9]$'),
                message=(
                    'Name must have only alphanumeric chars, "_", and ".".'),
                code='invalid_name',
            ),
        ]
    )       
    kind = models.CharField(max_length=2, choices=KIND_CHOICES, 
        verbose_name='Kind', help_text='Box version kind.', default="vb")  
    description = models.TextField(blank=True,
